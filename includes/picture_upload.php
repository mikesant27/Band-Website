...
if (isset($conn)) {
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if (!empty($image)) {
            $target_dir = "../includes/pictures/";  // Directory where images will be saved
            $target_file = $target_dir . basename($image);
            $image_entry = "../../../includes/pictures/" . basename($image);
            ...
            if ($check === false) {
                ...
                header('Location: ../includes/show_message.php?type=Upload Picture&message=' . urlencode($message));
                exit();
            }
            ...
            if (move_uploaded_file($_FILES['image']['tmp_name'], $target_file)) {
                ...
                $sql = "INSERT INTO pictures (title, image_path) VALUES (:title, :image_path)";
...
header('Location: ../includes/show_message.php?type=Upload Picture&message=' . $message);
